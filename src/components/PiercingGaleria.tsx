...
import { piercingGaleria } from "@/data/piercing"
...
function getCardWidth() {
  ...
  const el = document.querySelector<HTMLDivElement>(".piercing-galeria-card")
  return (el?.offsetWidth ?? 400) + GAP
}

export default function PiercingGaleria() {
  ...
  const itemsPerSet = piercingGaleria.length
  const extendedGaleria = [...piercingGaleria, ...piercingGaleria, ...piercingGaleria]
  ...
  return (
    <section className="py-section-gap overflow-hidden bg-surface-container-low" id="galeria-perforacion">
      <div className="px-6 md:px-margin-edge max-w-container-max mx-auto mb-16">
        <h2 className="section-title text-center tracking-widest">GALERÍA</h2>
        <p className="text-center text-secondary font-body-md mt-4 uppercase tracking-[0.2em] text-sm">
          Nuestros trabajos de perforación
        </p>
      </div>
      <div
        ref={scrollRef}
      ...
      >
        {extendedGaleria.map((src, i) => (
          <div
            key={`${src}-${i}`}
            className="carousel-item piercing-galeria-card relative group aspect-[3/4] transition-transform duration-500 group-hover/carousel:scale-[0.96] hover:!scale-105 z-0 hover:z-10 cursor-pointer select-none"
            onClick={() => openLightbox(i % itemsPerSet)}
          >
            <Image
              alt={`Perforación ${(i % itemsPerSet) + 1}`}
              fill
      ...
      {lightboxIndex !== null && (
        ...
        >
          ...
          >
            <Image
              alt={`Perforación ${lightboxIndex + 1}`}
              fill
              className="object-contain"
              src={piercingGaleria[lightboxIndex]}
              sizes="90vw"
              priority
            />
          </div>
